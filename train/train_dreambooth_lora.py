        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                if args.pre_compute_text_embeddings:
                    ...
                    prompts = args.validation_prompt.split('[EOS]')
                    for prompt in prompts:
                        pipeline_args.append({"prompt": prompt})

                images = []
                if args.validation_images is None:
                    for i in range(args.num_validation_images):
                        for cur_args in pipeline_args:
                            with torch.cuda.amp.autocast():
                                image = pipeline(**cur_args, generator=generator).images[0]
                            images.append({
                                "image": image,
                                "prompt": cur_args["prompt"],
                                "iteration": i
                            })
                else:
                    for i, image in enumerate(args.validation_images):
                        image = Image.open(image)
                        for cur_args in pipeline_args:
                            with torch.cuda.amp.autocast():
                                image = pipeline(**cur_args, image=image, generator=generator).images[0]
                                images.append({
                                    "image": image,
                                    "prompt": cur_args["prompt"],
                                    "iteration": i
                ...
                for tracker in accelerator.trackers:
                    if tracker.name == "tensorboard":
                        np_images = np.stack([np.asarray(img["image"]) for img in images])
                        tracker.writer.add_images("validation", np_images, global_step, dataformats="NHWC")
                    if tracker.name == "wandb":
                        tracker.log(
                            {
                                "validation": [
                                    wandb.Image(image["image"], caption=f'{image["iteration"]}: {image["prompt"]}') for image in images
                                ]
                            }
                        )
            ...
            for tracker in accelerator.trackers:
                ...
                if tracker.name == "wandb":
                    tracker.log(
                        {
                            "test": [
                                wandb.Image(image["image"], caption=f'{image["iteration"]}: {image["prompt"]}')
                                for image in images
                            ]
                        }
                    )
