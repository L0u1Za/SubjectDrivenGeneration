            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        unet.to(torch.float32).save_attn_procs(save_path, safe_serialization=not args.no_safe_serialization)
                        save_new_embed(
                            text_encoder,
                            modifier_token_id,
                            accelerator,
                    ...
                    for i in range(args.num_validation_images):
                        for prompt in prompts:
                            with torch.cuda.amp.autocast():
                                image = pipeline(prompt, num_inference_steps=25, generator=generator, eta=1.0).images[0]
                            images.append({
                                "image": image,
                                "prompt": prompt,
                                "iteration": i
